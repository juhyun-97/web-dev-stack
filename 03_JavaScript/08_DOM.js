...
// name으로 가져오기 : 같은 name속성이 여러개일 수 있으므로 배열로 반환
function btn4() {
...
function btn7() {
  const div = document.querySelector("#testId");
  // 속성 추가
  div.setAttribute("data-test", "테스트"); // 앞은 속성명,뒤는 속성값이 들어감
  // 속성값 가져오기
  console.log(div.getAttribute("data-test"));
  console.log(div.getAttribute("class")); // 속성명을 작성하면 속성값을 가져옴
  // 속성 삭제
...
const div2 = document.querySelector("#testId2"); // 함수바깥으로 빼놓으면 전역에서 선택가능

function btn9() {
  div2.classList.add("black"); // black 클래스 추가 후 html에서 스타일지정
}
...
function btn11() {
  ...
  //   if (div2.classList.contains("black")) {
  //     console.log(div2.classList.remove("black"));
  //   } else {
  //     console.log(div2.classList.add("black"));
  //   }
