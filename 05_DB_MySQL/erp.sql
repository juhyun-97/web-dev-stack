...
CREATE TABLE USER_INFO(
    ...
    PASSWORD VARCHAR(300) NOT NULL,  -- 암호화 되서 나와서 길게 처리함
...
-- 테이블 필요한 컬럼 짜기 힘들면 어떤 기능이 있어야 되는지 정도로 제출해도 괜찮아요!
DROP TABLE EMPLOYEE;
DROP TABLE PROJECT;
...
CREATE TABLE PROJECT_PERSON(
	PROJECT_PERSON_NO INT PRIMARY KEY AUTO_INCREMENT,
    PROJECT_PERSON_NAME VARCHAR(100) NOT NULL
);
ALTER TABLE EMPLOYEE ADD FOREIGN KEY (PROJECT_NO) REFERENCES PROJECT(PROJECT_NO);
ALTER TABLE EMPLOYEE ADD FOREIGN KEY (PROJECT_PERSON_NO) REFERENCES PROJECT_PERSON(PROJECT_PERSON_NO);