...
/*
    ...
    MySQL
    - 데이터 타입
    1. 문자actoractor_idfirst_nameaddress
...
CREATE TABLE SCHEDULE_MEMBERS(
	SCH_MOM_NO INT AUTO_INCREMENT PRIMARY KEY,
    USER_NO INT,
    SCHEDULE_NO INT
);

-- 실행하고자하는 쿼리문들 한꺼번에 실행하고 싶을 때
-- 해당 쿼리문들 같이 선택 후 CTRL + SHIFT + ENTER

-- 고객관계관리, 인적자원관리, 재무관리, 품질보증관리 -- 품질보증관리
-- 가장 해볼만 하다 싶은거 1개만 해도되고 아이디어 있으면 여러개 해도 무방!

-- CREATE TABLE QUALITY(
-- 	QUALITY_NO INT AUTO_INCREMENT PRIMARY KEY,
--     ITEM_NAME VARCHAR(50) NOT NULL, -- 구입물품명
--     PURCHASE_DATE DATE, -- 구입날짜
--     EXPIRATION_DATE DATE, -- 유통기한
--     WARRANTY VARCHAR(20) NOT NULL, -- 품질보증기간
--     STATUS VARCHAR(20) CHECK (STATUS IN ('정상', '불량', '조치필요')), -- 물품상태확인
--     PRO_NO INT,
--     USER_NO INT
-- );

-- CREATE TABLE QUALITY_MEMBERS(
-- 	QUALITY_MEM_NO INT AUTO_INCREMENT PRIMARY KEY,
--     USER_NO INT,
--     QUARLITY_NO INT
-- );
-- ALTER TABLE QUALITY ADD
-- FOREIGN KEY (PRO_NO) REFERENCES QUALITY(PRO_NO);
-- ALTER TABLE QUALITY ADD
-- FOREIGN KEY (USER_NO) REFERENCES QUALIRY(USER_NO);

-- ALTER TABLE QUALITY_MEMBERS ADD
-- FOREIGN KEY (USER_NO) REFERENCES QUALIRY_MEMBERS(USER_NO);
-- ALTER TABLE QUALITY_MEMBERS ADD
-- FOREIGN KEY (QUARLITY_NO) REFERENCES QUALIRY_MEMBERS(QUARLITY_NO);
...
CREATE TABLE HRM_LEAVES(
    ...
    CREATE_DATE DATETIME filmDEFAULT (CURRENT_DATE),
    USER_NO INT
);
