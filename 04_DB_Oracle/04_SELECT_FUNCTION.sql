/*
   ...
   >> 단일행 함수와 그룹 함수는 SELECT 절에서 함께 사용할 수 없음! 중복사용불가
...
--날짜 포맷 변경
ALTER SESSION SET NLS_DATE_FORMAT = 'YYYY-MM-DD HH:MI:SS'; -- 시간,분,초 추가
...
FROM DUAL; --  현재 설정된 화폐 단위에 따라서 바뀜
...
SELECT 
    ...
    TO_CHAR(SYSDATE, 'DD'), -- 일 : D - 주 (일요일 - 1, ...), DD - 월, DDD - 년
...
SELECT TO_NUMBER('100,000', '999,999') -- 999는 포맷
...
SELECT
    SUM (CASE WHEN DEPT_CODE = 'D5' THEN SALARY * 12 ELSE 0
    END)
FROM EMPLOYEE;
