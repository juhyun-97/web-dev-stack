...
-- 2. 뷰에 포함되지 않은 컬럼 중에서 베이스가 되는 컬럼이 NOT NULL 제약조건이 지정된
CREATE OR REPLACE VIEW VW_JOB
AS SELECT JOB_CODE FROM JOB;
...
UPDATE VW_JOB
SET JOB_NAME = '알바'
WHERE JOB_NAME = '사원'; --JOB_NAME이 VIEW에 존재하지 않아서 에러!
...
-- WITH READ ONLY : 뷰에 대해 조회만 가능(DEL 수행불가)
