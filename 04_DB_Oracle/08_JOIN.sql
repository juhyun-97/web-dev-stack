...
-- 1. 테이블명을 이용
-- 오라클 구문
SELECT EMP_ID, EMP_NAME, JOB.JOB_CODE, JOB_NAME FROM EMPLOYEE, JOB
...
*/
-- 사원사번, 사원명, 사원부서코드, 사수사번(MANAGER_ID), 사수명, 사수부서코드 조회
SELECT EMP_ID, EMP_NAME, DEPT_CODE, MANAGER_ID
FROM EMPLOYEE; -- 사원
SELECT EMP_ID, EMP_NAME, DEPT_CODE FROM EMPLOYEE; -- 사수

SELECT E.EMP_ID 사번, E.EMP_NAME 사원명, E.DEPT_CODE 사원부서코드, E.EMP_ID 사수사번, M.EMP_NAME 사수명, M.DEPT_CODE 사수부서코드
FROM EMPLOYEE E
...
/*
    ...
    - 두 테이블의 행들이 모두 곱해진 행들의 조합이 출력 -> 방대한 데이터 출력으로 과부화
*/ 
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE 
...
SELECT EMP_NAME, EMP_NO, DEPT_CODE, JOB_CODE FROM EMPLOYEE;
SELECT JOB_CODE, JOB_NAME FROM JOB;
SELECT DEPT_ID, DEPT_TITLE, LOCATION_ID FROM DEPARTMENT;

SELECT EMP_NAME 직원명, EMP_NO , DEPT_TITLE 부서명, JOB_NAME 직급명
FROM EMPLOYEE
    LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
    LEFT JOIN JOB USING (JOB_CODE)
...
SELECT EMP_NAME, DEPT_CODE, BONUS, SALARY*12 FROM EMPLOYEE;
SELECT LOCAL_CODE, NATIONAL_CODE, LOCAL_NAME FROM LOCATION;
SELECT DEPT_ID, DEPT_TITLE, LOCATION_ID FROM DEPARTMENT;
...
SELECT NVL(DEPT_TITLE, '부서없음') 부서명, TO_CHAR(ROUND(AVG(SALARY)), '999,999,999') 평균급여
FROM EMPLOYEE
     LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
GROUP BY DEPT_TITLE;
...
SELECT DEPT_TITLE 부서명, SUM(SALARY) 급여의합
FROM EMPLOYEE
 JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
 GROUP BY DEPT_TITLE
...
SELECT DEPT_ID, DEPT_TITLE, LOCATION_ID FROM DEPARTMENT;
SELECT SAL_LEVEL, MIN_SAL, MAX_SAL FROM SAL_GRADE;
SELECT GRADE_CODE, GRADE_NAME FROM MEM_GRADE;
...
SELECT EMP_NAME, DEPT_CODE, BONUS, SALARY FROM EMPLOYEE;
SELECT LOCAL_CODE, NATIONAL_CODE, LOCAL_NAME FROM LOCATION;
SELECT DEPT_ID, DEPT_TITLE, LOCATION_ID FROM DEPARTMENT;
SELECT JOB_CODE, JOB_NAME FROM JOB;
