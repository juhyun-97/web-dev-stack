...
-- 전 직원의 평균 급여보다 더 많은 급여를 받는 사원들 조회  전직원의 평균 급여 조회
SELECT AVG(SALARY)
...
SELECT EMP_ID, EMP_NAME, DEPT_CODE, MANAGER_ID, JOB_CODE,
...
*/
-- 과장 직급임에도 불구하고 차장 직급의 최대 급여보다 더 많이 받는 직원들 조회
SELECT SALARY
FROM EMPLOYEE
    JOIN JOB USING(JOB_CODE)
WHERE JOB_NAME = '차장'; -- 2800000   , 3760000
...
/*
    3. 다중열 서브쿼리
    - 서브쿼리의 조회 결과값이 한 행이지만 컬림이 여러개일 때 (한 행 여러 열)
*/
...
--SELECT ROWNUM, DEPT_CODE, ROUND(AVG(NVL(SALARY, 0)))
--FROM (SELECT ROWNUM, DEPT_CODE, SALARY 
--        FROM EMPLOYEE
--      ORDER BY SALARY DESC)
--WHERE ROWNUM <= 3;
...
SELECT TITLE
FROM MOVIE
WHERE DIRECTOR = ANY (SELECT DIRECTOR
FROM MOVIE
WHERE TITLE = '국제시장');
