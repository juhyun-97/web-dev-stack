/*
    ...
    - SQL 종류
       1. DML(Data Manipulation Language) : 데이터 조작어 ////중요!!
...
/*
       ...
       - 조회하고자 하는 컬럼은 반드시 FROM절에 지정한 테이블에 존재해야 한다.
...
-- EMPLOYEE에서 직원명(EMP_NAME), 월급(SALARY), 연봉조회
...
/*
   ...
   - 주의사항 : DISTINCT는 SELECT절에서 한 번만 기술 가능 중복사용불가
*/
...
-- EMPLOYEE에서 부서코드가 (DEPT_CODE)가 'D9'인 사원들 (모든 컬럼) 조회
SELECT * 
FROM EMPLOYEE
WHERE DEPT_CODE = 'D9'; 
...
FROM EMPLOYEE
WHERE BONUS IS NULL;
-- 부서배치를 아직 받지 않고(DEPT_CODE) 보너스(BONUS)는 받는 사원들 조회
SELECT *
FROM EMPLOYEE
...
-- EMPLOYEE에서 전체사원의 사원명,보너스 조회(보너스 기준 오름차순)
...
-- EMPLOYEE : 성별 사원 수 조회 000000-0000000
SELECT 
