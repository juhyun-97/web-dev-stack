...
public class A_ArrayList {
	...
	public void method1() {
		...
		//list.add(1); int 타입이라
		list.add("이승민");
		list.add(null); // String 기본값이라
		//list.add(true); boolean 타입이라
	...
	public void method2() {
		...
		// 1. add : 배열의 추가
		list.add(new Person("전현무", "삼성동", 47));  // 1번째 [0]
		list.add(new Person("박나래", "이태원동", 39)); // 2번째 [1]
		// 인덱스를 지정하여 원하는 해당 인덱스에 추가 
		// --> 내부적으로 기존에 있는 것들은 뒤로 밀리고 해당 인덱스에 추가
		list.add(1, new Person("기안84", "과천시", 40)); // 3번째 [2] // 2번째로 위치 변경
		
		// 2. set : 해당 인덱스의 값 변경
		list.set(0, new Person("키", "한남동", 33)); // [0]에 해당하는 사람 정보 변경하고싶을때
		
		// 3. size : 리스트 안에 몇 개의 데이터가 있는지
		System.out.println("사람 수 : " + list.size());
		
		// 4. remove : 해당 인덱스의 객체 삭제
		list.remove(1); // [1]에 있는 정보 삭제하기
	    ...
	    List<Person> subList = list.subList(0, 2); // 어디배열 어디배열까지 자를거냐(배열 0부터 배열 2 앞에까지~)
	    System.out.println(subList);
	    
	    // 7. addAll : 컬렉션을 통째로 추가
	    list.addAll(subList); // (sublist) 처리한 리스트를 뒤에 추가로 배열
	    
	    // 8. isEmpty : 컬렉션이 비어있는지
	    System.out.println("리스트가 비어있는지? " + list.isEmpty()); // 값이 있기때문에 비어있지않다
	...
	public void method3() {
		...
		// 10. 오름차순 정렬 : Comparable 인텊이스를 구현하고 있는 요소를 가지고 비교값들 반환하여 정렬
		Collections.sort(list);
		// 11. 내림차순 정렬 : 정확하게 내림차순 정렬하려면 먼저 오름차순(sort) 처리 후 내림차순(reverse) 정렬 
